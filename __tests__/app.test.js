...
describe('animal routes', () => {
  ...
  let testAnimal;
  beforeEach(async () => {
    const { body } = await (await request(app).post('api/v1/animals')).send({
      name: 'critter',
      dayOrNight: 'night',
      furry: true,
      numberOfLegs: 5,
  ...
  it('should add an animal', async () => {
    const res = await (await request(app).post('api/v1/animals')).send({
      name: 'skipper',
      dayOrNight: 'day',
      furry: false,
      numberOfLegs: 0,
    ...
    expect(res.body).toEqual({
      ...
      dayOrNight: expect.toEqual('day' || 'night'),
